    ...
    let yt_dlp_path = match get_yt_dlp_path() {
        ...
        None => {
            ...
            match get_yt_dlp_path() {
                ...
                None => return Err("yt-dlp was not found after installation. Please install it manually.".into())
            }
        }
    };
...
pub fn fetch_playlist_videos(url: &str) -> Result<Vec<Music>, Box<dyn std::error::Error>> {
    ...
    if let Some(playlist) = output.clone().into_playlist() {
        println!("Found playlist: {}", playlist.title.unwrap_or_else(|| "Unknown".to_string()));
...
pub fn download_video(video: &Music, download_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    youtube_dl.download_to(download_dir);
    ...
    match status {
        Ok(_) => {
            ...
            println!("✓ Downloaded {} in {:.2}s", title, duration.as_secs_f32());
            Ok(())
        }
        Err(e) => {
            println!("✗ Error downloading {}: {}", title, e);
            Err(e.into())
        }
    }
} 